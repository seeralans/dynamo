...
def draw_fins_between_two_curves(upper, lower, scalars, fig, **kwargs):
  ...
  tris.extend([(i + num_centroids + 1, i + 1, i + num_centroids) for i in range(num_centroids - 1)])
  mlab.triangular_mesh(*road.T, tris, scalars=np.hstack((scalars, scalars)), figure=fig, **kwargs)
...
def draw_helices(hels, fig, hel_radius=1, hel_opacity=1.0, hel_colour=(1.0, 1.0, 1.0), **kwargs):
  """
  ...
  for i, h in enumerate(hels):
    mlab.plot3d(*h.T, tube_radius=hel_radius, opacity=hel_opacity, color=hel_colour, figure=fig, **kwargs)
    
def visualise_centroids(means, variances, fig, centroid_tube_colour=(1.0, 1.0, 1.0), interp_points=None, fin_dims=[0, 1]):
  """
  ...
  # plot centroids
  mlab.points3d(*means.T, scale_factor=1, scale_mode="none", figure=fig, color=(1.0, 1.0, 1.0))

  # plot centroids tube
  mlab.plot3d(*smooth_it(means).T, tube_radius=0.1, color=centroid_tube_colour, figure=fig)
  ...
  for dim in fin_dims:
    ...
    draw_fins_between_two_curves(smooth_it(upper), smooth_it(lower), smooth_it(eigvals[:, 0][:, None])[:, 0], fig, colormap='viridis')
  return
