...
Dynamo: A Python Library for Coarse-Grained Modeling of Repeat Proteins
...
#+begin_src python
import dynamo
from dynamo import rp
#+end_src

#+RESULTS[ed8d74e900f03f7f7b95ad216d523dc74dae6a08]:
...
from dynamo import ProbPos, GeneralModule, Construct
pp.style.use(['default'])

rot_xy = lambda th: np.array([[np.cos(th), -np.sin(th), 0], [np.sin(th), np.cos(th), 0], [0, 0, 1]])
prob_pos = lambda mu, f_cov=np.zeros(3, float): ProbPos(
  np.array([mu]),
...
mod = lambda dist, th: GeneralModule(
  [
...
construct = Construct(general_mods, edges)
...
#+RESULTS[6e77da880efdad6e5458959f38b0f24cdabc529b]:
[[file:./.ob-jupyter/f46f89c6618dd5ae2c99ad31c3c2e02273ba101a.png]]

** Hull Generation

#+begin_src python
import numpy as np
from dynamo.rp import dynamo_rp as drp
...
from dynamo.rp import dynamo_rp as drp
from dynamo.rp import parameters as pm
from dynamo.rp import rp_utility as ut
from dynamo.rp import params_gen as pmg
import json
import numpy as np
import copy
...
** Protein Builds 
#+begin_src python
from dynamo import Construct, vis
import numpy as np
from dynamo.rp import dynamo_rp as drp
...
plotter = vis.create_plotter()
construct.propagate(0)
assembled_mods = construct.get_assembled_modules()
vis.visualise_centroids_of_modules(assembled_mods, plotter, interp_points=100, fin_dims=[0, 1, 2])
vis.show(plotter)
#+end_src
...
for i in range(4):
 ...
 vis.visualise_centroids_of_modules(chain_mods, plotter, interp_points=100)
vis.show(plotter)
#+end_src
...
** PDB Generation
#+begin_src python
from dynamo.rp import pdb_generation as pg 
#+end_src
